...
#     df = pd.merge(df, unmeth_data, on=["chr", "segment_start", "segment_end", "state"])
    ...
    #df.loc[(df["meth_count"] == 0) & (df["unmeth_count"] == 0), 'state'] = nd_state
    #df = clean_df(df)

    #liklihood_df = df.copy()
    #temp = calc_likelihood(liklihood_df)
    #print("Remove Useless data SSE: ", temp)

    # Segments where cpg_count == 0 -> Become no_data states
#     df.loc[(df['cpg_density'] == 0), 'state'] = nd_state
#     df = clean_df(df)
#     num_im_regions(df.copy(), im_state, og_chrom)
...
#     print("Chr7 mystery: ", df[(df["chr"] == "chr7") & (df["segment_start"] >= 61816200) & (df["segment_end"] <=61822200)] )
...
#     print("Chr7 mystery: ", df[(df["chr"] == "chr7") & (df["segment_start"] >= 61816200) & (df["segment_end"] <=61832200)] )
    ...
    #8 - Remove large IM bins
    #df.loc[(df['segment_length'] >= 15000) & (df['state'] == im_state), 'state'] = nd_state
    #df = clean_df(df)
    #num_im_regions(df.copy(), im_state, og_chrom)
    ...
    #df_um = df[(df['state'] == im_state) & (df['segment_length'] >=600) & (df['segment_length'] <=3000)]
    #sample_table = df_um.sample(n = 10000)
    #filename = 'um_samples.bed'
    #sample_table.to_csv(os.path.join(output_path,filename), sep="\t", index=False)
